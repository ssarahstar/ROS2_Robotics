...
def main():
    ...
    # ========================================
    # 1) Home Joint State로 먼저 이동
    # ========================================
    logger.info("--- Home으로 이동 시작 ---")
    ...
    # ========================================
    # 2) Waypoint 실행 (여러 지점 순차 이동)
    # ========================================
    # 이동하고자 하는 (x, y, z) 목표 지점 리스트 정의
    waypoints = [
        {'x': 0.4, 'y': 0.2, 'z': 0.5},
        {'x': 0.4, 'y': -0.2, 'z': 0.5},
        {'x': 0.5, 'y': 0.0, 'z': 0.5}
    ...
    for i, wp in enumerate(waypoints):
        logger.info(f"--- Waypoint {i+1} 이동 시작: x={wp['x']}, y={wp['y']}, z={wp['z']} ---")
        
        # 현재 상태를 시작점으로 갱신
        arm.set_start_state_to_current_state()
        
        # 목표 위치 갱신
        pose_goal.pose.position.x = wp['x']
        pose_goal.pose.position.y = wp['y']
        pose_goal.pose.position.z = wp['z']
